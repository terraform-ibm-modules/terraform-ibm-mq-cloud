...
data "local_sensitive_file" "bootstrap_api_key" {
  count    = fileexists("bootstrap_api_key.txt") ? 1 : 0
...
locals {
  api_key      = regexall("(.*api_key\":\")([0-9a-zA-Z_]*)", data.local_sensitive_file.bootstrap_api_key[0].content)[0][1]
  api_key_id   = nonsensitive(regexall("(.*api_key_id\":\")([0-9a-fApiKey-]*)", data.local_sensitive_file.bootstrap_api_key[0].content)[0][1])
  api_key_name = nonsensitive(regexall("(.*api_key_name\":\")([0-9a-zA-Z-_]*)", data.local_sensitive_file.bootstrap_api_key[0].content)[0][1])
}
