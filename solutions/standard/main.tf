...
# TODO: This is not the certificate, that is not available ATM. This is an href to the certificate.
# The comment lines are retained, since some will be required when the final format of the cert is known.
module "certificate_secret" {
  ...
  endpoint_type           = "public"
  secret_name             = "mq-da-cert-${local.queue_manager_name}"
...
module "root_certificate_secret" {
  ...
  secret_group_id         = local.secret_group_id
  endpoint_type           = "public"
  secret_name             = "mq-da-root-cert-${local.queue_manager_name}"
  secret_description      = "MQ DA ${local.queue_manager_name}"
  secret_type             = "arbitrary"
